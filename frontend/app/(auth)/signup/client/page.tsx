...
function ClientSignupForm() {
  ...
  const handleNext = async (e: React.FormEvent) => {
    ...
    if (step === 1) {
      ...
      // If not google auth, we must verify email via OTP before proceeding to Step 2
      if (!isGoogleAuth) {
        try {
  ...
  const handleComplete = async () => {
    clearError();
    try {
      // User has already passed OTP or Google Auth, so register directly logs them in.
      await register(
        formData.fullName,
        formData.email,
